...
// === التوجيه الذكي ===
// يعمل فقط إذا كان المستخدم قد اختار "تثبيت الدخول"
...
function unlockApp() {
    localStorage.removeItem('app_mode'); // المدير يمسح وضع الزبون
...
window.restoreBackup = function(input) {
    ...
    if(!file) return;
    if(!confirm("⚠️ تنبيه: استعادة النسخة سيضيف البيانات الحالية. هل أنت متأكد؟")) return;
    ...
    reader.onload = async (e) => {
        try {
            ...
            if(data.customers) {
                for(const c of data.customers) await addDoc(collection(db, "customers"), c);
            }
            if(data.transactions) {
                for(const t of data.transactions) await addDoc(collection(db, "transactions"), t);
            }
...
window.changeAdminPassReal = function() {
    ...
    if(hashPass(old) !== localStorage.getItem('admin_pass')) return alert("كلمة المرور الحالية خطأ");
